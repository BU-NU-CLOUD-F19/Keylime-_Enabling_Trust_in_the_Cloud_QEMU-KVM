# Keylime: Enabling Trust in the Cloud - QEMU/KVM
 Video presentation: [Video link](https://www.youtube.com/watch?v=idVgJATigfg&feature=youtu.be) 
## Table of Contents
...
We will be implementing a Merkle tree within the provider verifier to store the nonces of quote requests that arrive either concurrently or while the hardware TPM is busy. Following batch turnover the provider verifier will send the root node, merkle proof, and signed quote back to the tenant verifiers. This setup is done asynchronously using the python libary AsyncIO.

Finally we need to figure out the registration process that will let both the tenant registrar to know where and which one is its provider using IP address. Furthermore it also need the public AIK key from the provider to validate quote from hardaware TPM. Before we implement the registrartion process, we will be hardcoding provider info inside the tenant.
...
The request for a provider quote happens in the bootstrapping process, so we add the new state `GET_PROVIDER_QUOTE`in front of the `PROVIDE_V` state. We also add fail-retry handling for this state.

Inside the verifier, we have a agent dictionary to store all the information about the agent. Inside the dictionary, we add a key-value pair `need_provider_quote` to differentiate whether the agent is associated with the provider or the tenant.  If the agent is a cloud node, that means it is the provider, so `need_provider_quote` is False, and it goes to the `PROVIDE_V` state directly. If the agent is a VM on a cloud node, the `need_provider_quote` is True, and it will go to the `GET_PROVIDER_QUOTE` state to ask for a quote from the provider. Also, because we haven't implemented the registration process, we manually enter the identity of the agent through the tenant terminal when provisioning the agent. 

Inside the state `GET_PROVIDER_QUOTE`, we send a GET request to the provider verifier, with parameters `nonce`, `mask` and `vmask`. Since the registration process has not been implemented yet, we use the IP address entered from the tenant to indicate where the provider is. The endpoint in the provider verifier process of the request will be discussed in the next section. After getting the provider quote back, the tenant verifier will validate the quote (discussed in the Verification of Quote section). If the quote is invalid, it will go to the `INVALID_QUOTE` state and stall there. Otherwise, it will continue following the bootstrapping process, and go to the `PROVIDE_V` state.
...
Building a Merkle Tree to collect nonces addresses the long latency of the hardware TPM. It usually takes hundreds of milliseconds to about one second to complete one cryptographic operation. In the current design, the provider verifier not only needs to forward all tenant verifiers' requests, but also its own requests when asking the hardware TPM for quotes. Due to this latency, these requests are very likely to be stalled, which greatly slows down the system. 

Our mentor, Nabil, had the idea to decrease the thoughput of accessing hardware the TPM. This would be done by aggregating nonces from all requests into a Merkle Tree while TPM is busy. Then, when the TPM is free, the verifier will use this batch to ask for a quote from the hardware TPM. Using this idea, we developed a solution for nonce aggregation. First, we used `asyncio.sleep` to simulate the stall on the TPM inside the provider verifier. During this period, all the nonces from tenant verifier's requests will be appended to a Merkle tree inside the provider verifier. After the sleep (when the TPM is ready), we use the root of the Merkle tree as a new nonce to ask for a single quote from its agent, who can communicate with the hardware TPM. Since the root contains information from all nonces, sending the root as the nonce acts as sending all nonces at once. Then, the provider agent will send a quote generated with the new nonce back to the provider verifier.
